...
It can be:
. Used locally via stdio with MCPStdioTool
. Hosted as HTTP endpoint for Azure AI Foundry's HostedMCPTool
...
def convert_currency(amount: float, from_currency: str, to_currency: str) -> str:
    ...
    try:
        ...
        return f""" {amount:,.f} {from_currency.upper()} = {data['conversion_result']:,.f} {to_currency.upper()}
 Rate:  {from_currency.upper()} = {data['conversion_rate']:.4f} {to_currency.upper()}
...
def estimate_flight_time(origin: str, destination: str) -> str:
    ...
    try:
        from math import radians, sin, cos, sqrt, atan, asin
        ...
        R = 67  # Earth radius in km
        lat, lon, lat, lon = map(radians, [origin_lat, origin_lon, dest_lat, dest_lon])
        
        dlat = lat - lat
        dlon = lon - lon
        a = sin(dlat/)** + cos(lat) * cos(lat) * sin(dlon/)**
        c =  * asin(sqrt(a))
        distance_km = R * c
...
def get_detailed_weather(location: str) -> str:
 ...
 Wind: {current['windspeedKmph']} km/h {current['winddir6Point']}
