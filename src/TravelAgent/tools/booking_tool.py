...
def search_hotel_destinations(query: str) -> str:
    ...
    if isinstance(result, list):
        ...
        return f"Unexpected response format. Response: {str(result)[:00]}"
@ai_function(
    name="search_hotels",
    ...
    - adults: Number of adult guests (default )
    
    Examples:
    - search_hotels("-45698", "city", "05-0-5", "05-0-8", )"""
)
def search_hotels(
    dest_id: str,
    dest_type: str = "city",
    ...
    Args:
        ...
        adults: Number of adults (default: )
        room_qty: Number of rooms (default: )
    """
    today = datetime.now()
    if not checkin_date or checkin_date < today.strftime("%Y-%m-%d"):
        checkin_date = (today + timedelta(days=)).strftime("%Y-%m-%d")
    ...
    if not checkout_date or checkout_date <= checkin_date:
        try:
            ...
            checkout_date = (checkin_dt + timedelta(days=)).strftime("%Y-%m-%d")
            logger.info(f"No valid checkout_date provided, using checkin +  days: {checkout_date}")
        except:
    ...
    try:
        ...
        if checkin_dt < today:
            logger.warning(f" Check-in date {checkin_date} is in the past! Adjusting to tomorrow")
            checkin_dt = today + timedelta(days=)
            checkin_date = checkin_dt.strftime("%Y-%m-%d")
            checkout_dt = checkin_dt + timedelta(days=)
            checkout_date = checkout_dt.strftime("%Y-%m-%d")
            logger.info(f" Adjusted dates - Check-in: {checkin_date}, Check-out: {checkout_date}")
        
        if checkout_dt <= checkin_dt:
            logger.warning(f" Check-out date {checkout_date} is before or equal to check-in, adjusting")
            checkout_dt = checkin_dt + timedelta(days=)
            checkout_date = checkout_dt.strftime("%Y-%m-%d")
            logger.info(f" Adjusted check-out to: {checkout_date}")
    except ValueError as e:
        logger.error(f" Invalid date format: {e}")
        return f"Error: Invalid date format. Please use YYYY-MM-DD (e.g., 05--0)"
    
    logger.info(f" Final validated dates being sent to API - Check-in: {checkin_date}, Check-out: {checkout_date}")
    
    endpoint = "/v/hotels/search"
    params = {
        "dest_id": dest_id,
        "dest_type": dest_type,
    ...
    if isinstance(hotels, list) and len(hotels) > 0:
        ...
        for hotel in hotels[:0]:  # Limit to 0 hotels
            ...
            if review_score:
                ...
                hotel_info.append(f"    Rating: {review_score}/0 {review_word} ({review_count} reviews)")
...
@ai_function(
    ...
    Examples:
    - "Hotel prices in Tokyo?"  get_hotel_prices_live("Tokyo", "05-0-5", "05-0-8")
...
def get_hotel_prices_live(
    ...
    dest_endpoint = "/v/hotels/locations"
    dest_params = {
        "name": location,
        "locale": "en-gb"
    }
    ...
    if not checkout_date or checkout_date <= checkin_date:
        try:
            checkin_dt = datetime.strptime(checkin_date, "%Y-%m-%d")
            checkout_date = (checkin_dt + timedelta(days=)).strftime("%Y-%m-%d")
            logger.info(f"Using default checkout_date (checkin +  days): {checkout_date}")
        except:
            checkout_date = (today + timedelta(days=4)).strftime("%Y-%m-%d")
...
def search_flights(
    ...
    return (
        f" Flight search from {origin} to {destination} ({departure_date}):\n\n"
        f" The Booking.com API subscription you have focuses on hotels.\n\n"
        f" For flight searches, you can:\n"
        f". Use Google Flights: https://www.google.com/flights\n"
        f". Use Skyscanner: https://www.skyscanner.com\n"
        f". Check airline websites directly\n"
...
def check_booking_api_status() -> str:
    ...
    result = _make_booking_api_request_sync("/v/static/country", {})
...
def get_supported_currencies() -> str:
    ...
    result = _make_booking_api_request_sync("/v/static/currency", {})
    
    if "error" in result:
        return f"Error: {result['error']}"
    
    if isinstance(result, list):
        currencies = [f"{c.get('currency', 'N/A')}" for c in result[:0]]
